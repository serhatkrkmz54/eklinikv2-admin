      ...
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          {details && (
            <DialogDescription>
              Tarih:{" "}
              {format(parseISO(details.appointmentTime), "dd MMMM yyyy, HH:mm")}
            </DialogDescription>
          )}
        </DialogHeader>
  ...
  if (!history || history.length === 0) {
    return (
      <p className="text-sm text-muted-foreground text-center">
        Hastanın geçmiş tıbbi kaydı bulunmamaktadır.
      </p>
    );
  }
    ...
    <div className="space-y-2">
      {history.map((item) => (
        <Button
          ...
          variant={
            item.appointmentId === currentAppointmentId ? "secondary" : "ghost"
          }
        ...
        >
          <p className="font-semibold text-sm">
            {format(parseISO(item.appointmentTime), "dd.MM.yyyy - HH:mm")}
          </p>
...
const MedicalRecordView = ({
  ...
  details: AppointmentDetailResponse;
}) => {
    ...
    return (
      <div className="text-center p-4 bg-green-50 rounded-lg">
        ...
        <p className="text-sm text-muted-foreground">
          Bu randevu için tıbbi kayıt bilgisi bulunamadı.
        </p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      <div>
        <h3 className="font-semibold text-lg mb-2 flex items-center">
          <FileText className="mr-2 h-5 w-5" /> Tanı ve Teşhis
        </h3>
      ...
      {record.notes && (
        <div>
          <h3 className="font-semibold text-lg mb-2 flex items-center">
            <StickyNote className="mr-2 h-5 w-5" /> Doktor Notları
          </h3>
          <p className="text-muted-foreground bg-muted p-3 rounded-md">
            {record.notes}
          </p>
      ...
      {record.prescriptions && record.prescriptions.length > 0 && (
        <div>
          <h3 className="font-semibold text-lg mb-2 flex items-center">
            <Pill className="mr-2 h-5 w-5" /> Reçete Edilen İlaçlar
          </h3>
      ...
      <Card className="h-full flex items-center justify-center">
        <CardContent className="text-center text-muted-foreground">
          <p className="font-semibold">
            Detayları görmek için bir randevu seçin.
          </p>
        </CardContent>
      </Card>
    );
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <InfoItem
                  label="Doğum Tarihi"
                  value={
                    details.patientDetails.dateOfBirth
                      ? `${format(
                          parseISO(details.patientDetails.dateOfBirth),
                          "dd.MM.yyyy"
                        )} (${age} yaşında)`
                      : "-"
                  }
                ...
                <InfoItem
                  label="Kilo / Boy"
                  value={`${details.patientDetails.weight || "-"} kg / ${
                    details.patientDetails.height || "-"
                  } cm`}
                />
                <InfoItem
                  label="E-posta"
                  value={details.patientDetails.email}
                />
                <InfoItem
                  label="Telefon"
                  value={details.patientDetails.phoneNumber}
                />
              </div>
            </div>
        ...
        <Card className="shadow-lg lg:col-span-1">
          <CardHeader>
            <CardTitle className="text-lg flex items-center">
              <History className="mr-2 h-5 w-5" /> Hasta Geçmişi
            </CardTitle>
          </CardHeader>
          <CardContent>
            {isLoadingHistory ? (
