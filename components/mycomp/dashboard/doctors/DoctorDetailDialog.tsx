...
import {
  DoctorResponse as OriginalDoctorResponse,
  useDoctorById,
} from "@/hooks/useDoctorService";
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
const InfoRow = ({
  ...
  <div>
    ...
    <p className="font-semibold text-foreground text-base ml-6">
      {value || "Belirtilmemiş"}
    </p>
  </div>
);
      ...
      <DialogContent className="sm:max-w-2xl p-0 overflow-hidden">
        <VisuallyHidden>
          ...
          <DialogHeader>
            <DialogTitle>
              {doctor
                ? `Doktor Detayı: ${doctor.user.firstName} ${doctor.user.lastName}`
                : "Doktor Detayları"}
            </DialogTitle>
            <DialogDescription>
              {doctor
                ? `${doctor.title} ünvanlı, ${doctor.clinic.name} kliniğinde görevli doktorun detaylı bilgileri.`
        ...
        {isLoading ? (
          ...
          <div className="w-full">
            <div className="flex flex-col items-center gap-2 p-6 text-center bg-muted/50 border-b">
              ...
              <Badge
                variant="outline"
                className="text-base font-medium border-sky-300 bg-sky-50 text-sky-800"
              >
                {doctor.title}
              </Badge>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <InfoRow
                    ...
                    value={doctor.user.nationalId}
                  />
                  <InfoRow
                    icon={ShieldCheck}
                    label="Kullanıcı Rolü"
                    value={getRoleInTurkish(doctor.user.role)}
                  />
                  <InfoRow
                    icon={Calendar}
                    label="Kayıt Tarihi"
